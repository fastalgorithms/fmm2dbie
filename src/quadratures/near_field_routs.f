...
c 
c              findnear2dmem - memory management routine for find
c                            near
c
c              O(n^2) analogs of findnear and findnearmem
c
c              findnear2dslow - find near field of a collection
c                         of sources with extent and targets
c 
c              findnear2dslowmem - memory management routine for find
c                            near
c
c              get_iquad_rsc2d - given a row sparse compressed
...
c
c              get_quadparams_adap2d - get various parameters
...
c               target info.
c               The first two components must be xy coordinates
c
c       nt      in: integer
c               number of targets
c
...
c-------------------------------
      ...
      real *8 xys(2,ns),targets(ndt,nt),rads(ns)
      ...
      do i=1,nt
        nlst(i) = 0
        do j=1,ns
          rr = (xys(1,j)-targets(1,i))**2 + 
     1          (xys(2,j)-targets(2,i))**2 
...
c------------------------------------------------------------------      
      ...
      subroutine findnear2dslowmem(xys,ns,rads,ndt,targets,nt,nnz)
c
...
c
c------------------------------------------------------------------      
      subroutine findnear2dmem(xys,ns,rads,ndt,targets,nt,nnz)
c
cc      identify all sources which are contained in 
c       |xys(:,i)-targets(:,j)|<=rads(i).
...
C$OMP END PARALLEL DO       
     ...
     1  ifunif,iper,nlevels,nboxes,ltree)
       ...
       call pts_tree_build(xys,ns,targs,nt,idivflag,ndiv,nlmin,
     1   nlmax,ifunif,iper,nlevels,nboxes,ltree,itree,ipointer,centers,
     2   boxsize)
       
       allocate(isrcse(2,nboxes),itargse(2,nboxes))
       allocate(isrcper(ns),itargper(nt))

       call pts_tree_sort(ns,xys,itree,ltree,nboxes,nlevels,
...
c 
               xdis = abs(centers(1,lbox)-xys(1,isrc))
               ydis = abs(centers(2,lbox)-xys(2,isrc))

               llev = itree(ipointer(2)+lbox-1)
               distest = rads(isrc)+boxsize(llev)/2
               if((xdis.le.distest).and.(ydis.le.distest)) then
                 
                 do itt=itargse(1,lbox),itargse(2,lbox)
                   itarg = itargper(itt)
                   rr = (xys(1,isrc)-targs(1,itarg))**2 + 
     1                (xys(2,isrc)-targs(2,itarg))**2  
                   if(rr.le.rtest) then
                     nnz = nnz + 1
                   endif
                 enddo
...
c
c-----------------------------------------------------
      subroutine findnear2d(xys,ns,rads,ndt,targets,nt,row_ptr,
     1       col_ind) 
c     
cc      identify all sources which are contained in 
c       |xys(:,i)-targets(:,j)|<=rads(i).
...
c
      implicit none
      integer npatches,ixys(npatches+1),npts,nnz
      ...
      do i=1,nnz
        ipatch = col_ind(i)
        npols = ixys(ipatch+1)-ixys(ipatch)
        iqtmp(i) = npols
      enddo
...
c
c       If h is the radius of the bounding disk,
...
c
      ...
      subroutine get_quadparams_adap2d(eps,nqorder,eps_adap,nlev,
     1   nqorder_f)
c
c
c
...
c
c
      subroutine get_near_corr_max2d(ntarg,row_ptr,nnz,col_ind,npatches,
     1   ixys,nmax)
c------------------------------------
c
...
c  sources by sending in ixys corresponding to oversampled
c  discretization
c
c
c  Input arguments:
...
c  - ixys: integer(npatches+1)
c      ixys(i+1)-ixys(i) is the number of discretization
c      nodes on patch i
c  
c  Output arguments:
c  
...
c-------------------------------
      ...
      integer, intent(in) :: ixys(npatches+1)
...
C$OMP$REDUCTION(max:nmax) 
      do i=1,ntarg
        ...
        do j=row_ptr(i),row_ptr(i+1)-1
          jpatch = col_ind(j)
          ntmp = ntmp + ixys(jpatch+1)-ixys(jpatch)
        enddo

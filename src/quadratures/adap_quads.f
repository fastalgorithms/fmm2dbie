...
c
      ...
      subroutine dgetnearquad_adap_guru2d(nch,norders,ixys,iptype,npts,
     1   srccoefs,srcvals,ndtarg,ntarg,targvals,ich_id,ts_targ,eps,ipv,
     2   fker,ndd,dpars,ndz,zpars,ndi,ipars,nnz,row_ptr,col_ind,iquad,
     3   nquad,wnear)
c
...
c
      ...
      kmax = maxval(norders)
      allocate(dints(kmax+5),dtmp(kmax+5))
...
C$OMP$PRIVATE(ts,umat,vmat,wts,kk,dtmp)
      do ich=1,nch
        kk = norders(ich)
        allocate(ts(kk),umat(kk,kk),vmat(kk,kk),wts(kk))
        call legeexps(itype,kk,ts,umat,vmat,wts)
     ...
     1          ndtarg,ntarg0,targvals(1,itarg),norders(ich),fker,
     2          ndd,dpars,ndz,zpars,ndi,ipars,nqorder,nchmax,dints)
            else
              call dget_adap_lege_self(ipv,eps,norders(ich), 
     1          ts_targ(itarg),srccoefs(1,istart),ndtarg,
     1          targvals(1,itarg),norders(ich),fker,ndd,
     2          dpars,ndz,zpars,ndi,ipars,nchmax,dints)
            endif
          endif
          call dmatmatt(1,norders(ich),dints,norders(ich),umat,
     1       wnear(iquad(iper(j))))
        enddo
        deallocate(ts,umat,vmat,wts)
      enddo
...
c
c
      subroutine dget_adap_lege_self(ipv,eps,norder,tt0,srccoefs,ndtarg,
     1   xytarg,nporder,fker,ndd,dpars,ndz,zpars,ndi,
     2   ipars,nchmax,dintvals)
c
c   This subrourine computes the integrals
c      
c     \int_{-1}^{1} K(x_{i},\rho(t)) P_{n}(t) dsdt(t) dt \, ,
...
c-----------------------------------
      ...
      real *8, intent(out) :: dintvals(nporder)
...
cc      call load_selfquad(ipv,iord,nqorder,ts0,w0)
      ...
      maxrec = 0
      numit = 0
      call dadinrecm(ier,stack,a,b,norder,srccoefs,ndtarg,xytarg,
     1  nporder,fker,ndd,dpars,ndz,zpars,ndi,ipars,nqorder,ts0,w0,
     2     vals,nnmax,eps,dintvals,maxdepth,maxrec,numit,value2,
     3     value3)
      if(ier.ne.0) then
        print *, "on left interval:"
        print *, "adaptive integration in self thrashed"
      ...
      call dadinrecm(ier,stack,a,b,norder,srccoefs,ndtarg,xytarg,
     1  nporder,fker,ndd,dpars,ndz,zpars,ndi,ipars,nqorder,ts0,w0,
     2  vals,nnmax,eps,dtmp,maxdepth,maxrec,numit,value2,
     3  value3)

      subroutine zchunkints_lege(eps,norder,srccoefs,ndtarg,
     1   ntarg,xytarg,nporder,fker,ndd,dpars,ndz,zpars,ndi,
     2   ipars,nqorder,nchmax,zintvals)
c
...
c    
c   Output arguments:
c     - zintvals: complex *16 (nporder,ntarg)
...
c-----------------------------------
      ...
      complex *16, intent(out) :: zintvals(nporder,ntarg)
      ...
      do i=1,ntarg
        ...
        maxrec = 0
        numit = 0
        call zadinrecm(ier,stack,a,b,norder,srccoefs,ndtarg,xytarg(1,i),
...
c     - vals: complex *16 (np,maxdepth)
c         work array 1
c     - value2: complex *16 (np)
c         work array 2
c     - value3: complex *16 (np)
c         work array 3
c    
c   Output arguments:
c     - ier: integer
...
c
      ...
      integer ipars(ndi)
      real *8 ts0(m),w0(m)
      complex *16 vals(np,maxdepth),zints(np),value2(np),value3(np)
      external fker
      ...
      call zoneintmu(a,b,k,srccoefs,ndt,targ,np,fker,ndd,dpars,ndz,
...
c 
         ...
         call zoneintmu(stack(1,j),c,k,srccoefs,ndt,targ,np,fker,ndd,
     1      dpars,ndz,zpars,ndi,ipars,m,ts0,w0,value2)
         call zoneintmu(c,stack(2,j),k,srccoefs,ndt,targ,np,fker,ndd,
...
c 
          do jj=1,np
            zints(jj)=zints(jj)+value2(jj)+value3(jj)
          enddo
          j=j-1
c 
...
c
c
      subroutine zoneintmu(a,b,k,srccoefs,ndt,targ,np,fker,ndd,dpars,

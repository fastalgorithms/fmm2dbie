...
c
c     unit test of the real matrix builder (scalar problem)
c      
...
c     get boundary data
      ...
      do i=1,npts
        call l2d_slp(xyz_out,2,srcvals(1,i),0,dpars,0,zpars,0,
     1     ipars,ubdry(i))
      enddo
...
c     build system matrix (-1/2 I + D)
     ...
     3     sysmat,ier)
      ...
      call cpu_time(t1)
      nrhs=1
      call dgesv(npts, nrhs, sysmat, npts, ipiv, sigma, npts,
     1     info)
      call cpu_time(t2)
